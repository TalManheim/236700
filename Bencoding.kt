...
class Bencoding{
    companion object {
        ...
        private fun decodeInt (str:ByteArray) : Int{
        ...
        private fun extractEvenBracketsRange (str: ByteArray, startIndex: Int):IntRange{
            ...
            while (cntBrackets != 0 && current < str.size){
                ...
                when (ch){
                    'i' -> { val intRange = extractIntRange(str.toString(charset = Charsets.UTF_8).toByteArray(), current)
                            current = intRange.endInclusive }
                    'l' -> cntBrackets++
                    'd' -> cntBrackets++
                    'e' -> cntBrackets--
                    in ('0'..'9') -> { val stringRange = extractStringRange(str.toString(charset = Charsets.UTF_8).toByteArray(), current)
